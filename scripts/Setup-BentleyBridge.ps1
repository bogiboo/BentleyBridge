...
$directories = @(
    (Join-Path $RepositoryRoot 'src\BentleyBridge'),
...
Generički managed C# AddIn za OpenCities Map Ultimate 2025. Bridge se učitava u Bentleyjev proces i na ručnu naredbu `ETAZ RUN` čita strogo definirane JSON naloge.
...
Izvedbeni sadržaj mape `runtime` ne sprema se u Git. Izvorni kod, JSON shema, primjeri, konfiguracija i build skripte jesu verzionirani.
...
Write-NewTextFile (Join-Path $RepositoryRoot 'src\BentleyBridge\.gitkeep') ''
...
try {
    ...
    & gh repo view $GitHubRepository --json nameWithOwner 1>$null 2>$null
...
Write-Host 'Sljedeće: izgraditi Hello World AddIn i potvrditi MdlTaskID/autoload naredbom ETAZ RUN.' -ForegroundColor Green
